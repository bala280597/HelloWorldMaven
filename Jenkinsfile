pipeline{
    ...
    stages{
        stage('Sonar Code Analysis') {
            steps {
                ...
                withSonarQubeEnv('sonarqube') {
                      ...
                      sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=HelloWorldMaven -Dsonar.sources=src -Dsonar.java.binaries=./target/** "
        ...
        always { 
            sh """ 
                   python Build.py credentials('jenkins_password') bala2805/HelloWorldMaven ${JOB_NAME} ${BUILD_NUMBER} ${currentBuild.durationString.replace(' and counting', '')}
               """
        }
    }
}